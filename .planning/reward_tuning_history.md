...
**Fix 31**: Reverted the `b_drive` scale and offset to `20.0` (positive). *(Update 2026-08-13: The F1Tenth USD model's physics root X-axis points towards the REAR WING, completely opposite to the visual mesh's nose. Because the root X-axis points backward, driving nose-first (positive scale) causes the physics engine to report the car is moving in its local `-X` direction. This broke the RL waypoint progress tracking which relied on root `yaw`. We fixed this by adding `+ pi` to the extracted `yaw` in `observations.py` to artificially rotate the root to match the visual nose. The scale remains at `20.0`).*
...
### Issue 54: The Crawling Local Minimum (79 steps, 0.25 m/s)
- **Problem:** At 500k timesteps, the agent was surviving for only ~79 steps and dying at ~0.25 m/s, causing the rl-watchdog to terminate the session due to stagnation. The root cause was a legacy check in `mdp/terminations.py` which assumed the car was driving backward (-X). Because the v1.0-working patch flipped the car to drive in +X, the `moving_forward` check (`vel < -0.05`) always failed. Once the 20-step spawn grace period expired, any contact with a track gate falsely triggered a fatal `gate_hit` termination, capping episode length at exactly the distance to the first gate (~79 steps).
- **Fix:** Updated the `moving_forward` check in `white_line_contact` to `vel > 0.05` to match the +X forward driving direction.
- **Result:** Checkpoints wiped and training restarted to allow the agent to progress past the first gate.

### Issue 55: The Steering Lock Trap (Smoothness vs Steer Penalty)
- **Problem:** The agent steered in one direction and refused to recover, resulting in a crash at ~80 steps. The `action_rate_smoothness_reward` (-2.0) punished *changes* in steering, while `action_steer_penalty` (-0.5) punished holding the wheel. It was 4x more painful for the agent to straighten the wheel than to hold it at full lock, trapping it in a mathematical local minimum where recovering from a turn was worse than crashing.
- **Fix:** Reduced `action_rate_smoothness_reward` weight in `arcpro_env_cfg.py` from `2.0` to `0.1`. This allows the agent to straighten out and recover from bad angles without suffering a massive smoothness tax, while still gently discouraging high-frequency jitter.
- **Result:** Checkpoints wiped and training restarted.

### Issue 56: Driving Blind & Backwards (The Forklift Reversion)
- **Problem:** The agent was exhibiting erratic oversteer and crashing without making progress (`WPs_cum: 0`). The `b_drive` scale and offset in `arcpro_env_cfg.py` had reverted to `20.0`. Because the F1Tenth USD model requires negative velocity to move forward (+X), the agent was driving backwards. This meant the camera was pointing the wrong way (driving blind) and the steering wheels were in the rear (forklift steering), making it physically impossible to recover from turns.
- **Fix:** Restored `scale` and `offset` for `b_drive` to `-20.0`. Action `0.0` now properly commands `-20.0 rad/s` (forward cruising), aligning the movement with the camera and restoring front-wheel steering stability.
- **Result:** Checkpoints wiped and training restarted.

### Issue 57: Track Direction vs Spawn Direction Mismatch (`WPs_cum: 0`)
* **Symptom**: The agent's cumulative waypoint metric (`WPs_cum`) was perpetually 0, meaning it was never rewarded for forward progress, and its episode lengths never exceeded the initial straightaway.
* **Root Cause**: The track waypoints in `track_1x_waypoints` were sequentially ordered heading South at the spawn point, but the car was spawned facing North (`rot=(0.7071, 0, 0, 0.7071)`). As the car drove North, its waypoint index mathematically decreased. The IsaacLab environment's anti-cheat clamp logic interpreted decreasing indices as "driving backward" and clamped the reward and metric to 0.
* **Fix**: Implemented `Bi-Directional Track Support` in `observations.py`. The environment now computes `env.extras["track_dir"]` at spawn time by comparing the car's heading to the waypoint's heading. If the car spawns facing opposite to the waypoint sequence, it dynamically inverts the `delta_wp` calculation, the `lat_err` offsets, and the `head_err` angles for the RL policy.

### Issue 58: The Moonwalking Exploit (Reward Conflict & Teleportation Bug)
* **Symptom**: `WPs_cum` was jumping exactly between `0` and `304` or `305` every other step. The agent was failing to learn to turn.
* **Root Cause**: The Fix 57 for `observations.py` inverted `head_err` so the agent received +2.0 points for facing North (forward), but the `waypoint_progress_reward` inside `rewards.py` was *still using the raw, backwards track direction*. This created a reward conflict where the agent was rewarded for facing North but moving South, causing it to learn to drive in reverse (moonwalk). Because driving in reverse frequently led to crashing into walls, the agent strayed >2.0m from its local track segment. This triggered a `TrackManager` global search fallback which mistakenly snapped the waypoint index to a different section of track 19,521 waypoints away, yielding a false massive progress spike (`19521 / 64 envs = 305 waypoints average`).
* **Fix**: Modified `rewards.py` to properly multiply `delta_real` by `env.extras["track_dir"]`. This aligns the progress reward direction with the heading reward, incentivizing the agent to drive forward normally.

### Issue 59: Teleportation Math Exploit (WPs_cum 306) & Crawling Trap
* **Symptom**: The agent crawled endlessly at `0.13 m/s` (taking the `-15.0` stationary penalty) and randomly generated massive progress spikes (e.g. `WPs_cum = 306`), causing the watchdog script to kill the tmux session due to a "LOGIC BUG".
* **Root Cause 1**: While Issue 58 fixed the direction of `delta_real`, it didn't prevent `TrackManager` from doing a global search and snapping the index to a distant waypoint (e.g., across the lane) when the agent drifted > 2.0m from the centerline. This snapping produced massive `delta_real` values (e.g., thousands) in a single step, yielding astronomically false rewards.
* **Root Cause 2**: The stagnation termination was set to `speed < 0.1 m/s` for 1000 steps, while the stationary penalty triggered at `speed < 0.2 m/s`. The agent learned to crawl at 0.13 m/s, taking the stationary penalty but avoiding stagnation termination, trapping itself in a deep local minimum where it wouldn't die but wouldn't learn.
* **Fix**: 
  1. Clamped `delta_real` in `rewards.py` to `[-10.0, 10.0]` per step to make it physically impossible for track-snapping exploits to yield massive progress spikes.
  2. Aligned `stagnation_termination` to `< 0.2 m/s` (matching the stationary penalty threshold) and reduced the max crawling steps from 1000 to 400. This forcefully kills episodes where the agent is stuck crawling, accelerating learning by ending doomed trajectories early.
* **Result**: Wiped checkpoints and restarted training.

### Issue 60: Watchdog Teleportation False Positives (WPs_cum: 305/306)
* **Problem**: Even after clamping  in  (Issue 59),  continued to spike to 305, causing  to kill the training.
* **Root Cause**:  telemetry logging was computed separately in  (as ) using the raw un-clamped index jump from . When TrackManager snapped, the raw observation metric jumped, tricking the watchdog into seeing a teleportation exploit even though the agent was not actually rewarded for it.
* **Fix**: Refactored  to use  (which is safely clamped by ). This guarantees the logged telemetry accurately reflects the rewarded progress and perfectly neutralizes the watchdog false positive.
* **Result**: Wiped checkpoints and restarted training.

### Issue 60: Watchdog Teleportation False Positives (WPs_cum: 305/306)
* **Problem**: Even after clamping delta_real in rewards.py (Issue 59), WPs_cum continued to spike to 305, causing watchdog.py to kill the training.
* **Root Cause**: WPs_cum telemetry logging was computed separately in observations.py (as track_wp_delta) using the raw un-clamped index jump from spawn_wp_idx. When TrackManager snapped, the raw observation metric jumped, tricking the watchdog into seeing a teleportation exploit even though the agent was not actually rewarded for it.
* **Fix**: Refactored observations.py to use env.extras.get("cumulative_wp_index") (which is safely clamped by rewards.py). This guarantees the logged telemetry accurately reflects the rewarded progress and perfectly neutralizes the watchdog false positive.
* **Result**: Wiped checkpoints and restarted training.

### Issue 61: The Crawling Local Minimum (Agent Commits Suicide to avoid penalties)
* **Problem**: The agent was surviving only ~79 steps at 500k timesteps. The watchdog flagged this as a "crawling local minimum" and killed the training.
* **Root Cause**: The `stationary` penalty weight in `arcpro_env_cfg.py` was set to `15.0` (which is `-15.0` net per step if slow), while the `survival_bonus` was only `1.0`. When the agent drove slowly (<0.5m/s), it accumulated a massive net penalty of `-14.0` per step. Research on RL reward shaping confirms that if step penalties are too high compared to the rewards, the agent perceives the terminal state (crashing into a wall) as mathematically better than staying alive and bleeding points. The agent intentionally committed suicide at ~79 steps to minimize its suffering.
* **Fix**: Reduced `stationary` weight from `15.0` to `5.0`. This yields a milder net penalty of `-4.0` per step when slow, which still encourages forward movement but doesn't induce suicidal behavior.
* **Result**: Wiped checkpoints and restarted training.

### Issue 62: The Fear of Steering (Crawling at 0.09 m/s)
* **Problem**: After Fix 61, the agent stopped committing suicide at 79 steps, but at the 0.6M milestone it was crawling at `0.09 m/s` and surviving for ~172 steps before crashing straight into a wall.
* **Root Cause**: The agent fell into a deep "conservative local minimum". The `termination_penalty` (-500, weight 10.0) was so devastating that the agent became completely risk-averse, preferring to crawl. Furthermore, the `action_steer_penalty` (-0.5) punished the agent for attempting to steer. Because the agent was too afraid to steer (to avoid the step penalty) and too afraid to drive fast (to avoid the massive -500 crash penalty), it simply crawled straight forward until it inevitably hit the wall.
* **Fix**: Disabled `action_steer_penalty` (set weight to `0.0`) to allow the newborn agent to freely explore cornering without immediate punishment. Reduced `termination_penalty` weight from `10.0` to `2.0` (yielding a -100 crash penalty instead of -500) to balance the magnitudes and encourage the agent to take risks (speeding up) without being paralyzed by the fear of crashing.
* **Result**: Wiped checkpoints and restarted training.

### Issue 63: The Weak Gradient Trap (Lazily Crashing at 60 steps)
* **Problem**: After Fix 62 and letting the agent train to 1.2M steps, the agent was crashing at exactly ~60 steps. The speed slightly improved (0.16 m/s) and it was earning positive step rewards, but it never learned to steer to avoid the wall.
* **Root Cause**: By reducing the `termination_penalty` to 2.0 (-100) and the `boundary_penalty` to 0.4 (-40), crashing became too "cheap". The agent earned positive points for driving forward (up to +50 per step), making the penalty for hitting the wall negligible by comparison. Because the negative feedback for crashing and going off-track was so weak, the agent lacked the strong gradients necessary to compel it to learn how to steer. It was trapped in a local minimum where lazily driving forward and taking a small crash penalty was easier than learning complex cornering maneuvers. Furthermore, reducing the `stationary_penalty` to 5.0 in Fix 61 had revived the "Stationary Min-Max Trap" from Issue 27, where sitting still (-4.0/step) was mathematically safer than driving fast and crashing.
* **Fix**: Restored the strong negative gradients that successfully drove learning in previous iterations. `termination_penalty` was restored to 10.0 (-500), `boundary_penalty` to 0.6 (-60), and `stationary_penalty` to 10.0. The `action_steer_penalty` remains disabled (0.0) so the agent can still freely learn to steer without step punishments. This combination provides a massive penalty for crashing and sitting still, forcing the agent to steer and drive fast to survive.
* **Result**: Wiped checkpoints and restarted training.

### Issue 64: The Spinning Top Exploit (Farming Survival Time)
* **Problem**: At the new 0.6M milestone, the agent was surviving for an impressive `241` steps, but its speed was an abysmal `0.02 m/s` and its cumulative progress (`WPs_cum`) was `0`. The episode reward was roughly `-312`.
* **Root Cause**: Because `action_steer_penalty` was disabled in Fix 62, steering was "free." The agent discovered that if it drove in tight circles (spinning like a top), its physics velocity remained above the `0.2 m/s` threshold required to avoid the `-10` stationary penalty. By spinning in place, it avoided walls for a long time (241 steps), collecting the `+1.0` `survival_bonus` at every step, yielding +241 points. It eventually drifted into a wall and took the -500 crash penalty. This "spinning top" exploit was mathematically superior to driving straight (which yielded a crash at 60 steps). The agent had learned to optimize for "time survived" rather than "distance traveled."
* **Fix**: Completely disabled the `survival_bonus` (set weight to `0.0`). The agent now receives absolutely no points for merely existing. It can ONLY earn positive points by making forward progress along the waypoints (`progress_reward`). If it spins in circles, it earns 0 points and eventually crashes for -500. This perfectly aligns the agent's objective with our goal: distance traveled.
* **Result**: Wiped checkpoints and restarted training.

### Issue 65: The Spin-Out (Backwards Driving)
* **Problem**: After disabling `survival_bonus`, the agent reached 1.2M steps but was still crashing at exactly `64` steps with `0.19 m/s` speed. Crucially, its cumulative waypoint progress (`WPs_cum`) was `-7`, meaning it was driving backwards!
* **Root Cause**: Without the `survival_bonus`, the agent had no incentive to spin in tight circles. However, because `action_steer_penalty` was still disabled (to prevent the "Fear of Steering"), the agent's random neural noise caused it to violently jerk the steering wheel to full lock at the start of the episode. This instantly caused the car to "spin out" and face backwards. It then drove backwards (earning negative progress) until it hit the wall at 64 steps. Because we couldn't re-enable the steering penalty without terrifying the agent, there was nothing to encourage it to keep the car facing forward.
* **Fix**: Increased the `heading` alignment reward weight from `2.0` to `10.0`. This acts as a powerful "conditional survival bonus." The agent earns `+10` points per step for facing perfectly forward, and loses `-10` points per step for facing backward. This dense gradient forces the agent to keep the car straight and suppresses spin-outs naturally, without explicitly punishing the steering action itself (allowing it to still steer smoothly around corners).
* **Result**: Wiped checkpoints and restarted training.

### Issue 66: The Return of the Stationary Min-Max Trap
* **Problem**: After increasing the `heading` reward to `10.0`, the agent reached the 0.6M milestone but was only surviving for `125` steps, moving at an abysmal `0.06 m/s`, and earning `0` cumulative waypoints.
* **Root Cause**: The agent fell back into the "Stationary Min-Max Trap" (Issue 27). Because the agent could earn `+10.0` points per step simply by sitting perfectly still and facing forward, this perfectly canceled out the `-10.0` `stationary_penalty` it received for not moving. The net reward was `0` points per step. Mathematically, it didn't matter to the agent if it sat still for 400 steps (yielding 0 points and then dying) or slightly wiggled into a wall at 125 steps (yielding 0 points and then dying). It had no mathematical incentive to take the risk of driving fast!
* **Fix**: Edited `mdp/rewards.py` to multiply the `heading_alignment_reward` by the agent's physical speed (`torch.norm(vel)`). Now, the agent ONLY earns the `+10` heading reward IF it is physically moving fast. If it sits still (speed = 0), it earns 0 heading points, and the `-10` stationary penalty crushes its score. This mathematically forces the agent to drive straight AND fast to survive.
* **Result**: Wiped checkpoints and restarted training.

### Issue 67: Unkillable Spinning Top
* **Problem**: After the Fix 66 speed multiplier was applied, the agent reached the 0.6M milestone. It was surviving for exactly `401` steps (meaning it hit the stagnation termination limit), but its speed at death was `0.00` and its cumulative waypoint progress was `0`. The reward was roughly `-576.5`.
* **Root Cause**: Because the agent ONLY earned heading rewards by driving fast AND straight, and because driving straight meant crashing at 60 steps (yielding `+1020` points before suffering a `-500` crash, netting `+520` points), the agent found a way to "survive" longer. The random neural noise at the start of the episode made it difficult to drive perfectly straight for 60 steps. Instead, the agent realized that spinning in tight circles at a speed `> 0.2 m/s` allowed it to evade the `stationary_penalty`. It earned `0` heading reward and `0` progress reward, yielding a net score of `0` for 400 steps, before getting killed by the `stagnation_termination` (which dipped below 0.2 m/s due to oscillation) yielding a `-500` crash penalty and `-576.5` total reward. Because it lived longer and didn't crash early, it preferred spinning over risking a fast drive.
* **Fix**: Restored the `action_steer_penalty` to `-0.5` in `arcpro_env_cfg.py`. In Fix 62, this was disabled because it caused "Fear of Steering." However, the agent's environment is vastly different now. With the speed-multiplied heading reward and the massive stationary penalty, the agent is forced to drive fast. The `-0.5` steering penalty makes spinning in circles (-200 steering penalty + -500 termination = -700 points) mathematically worse than crashing early. The agent's ONLY profitable option now is to drive fast, face forward, and keep the wheel straight!
* **Result**: Wiped checkpoints and restarted training.

### Issue 68: The Gas Pedal Exploit (Survival Bonus in Disguise)
* **Problem**: After restoring the `action_steer_penalty` in Fix 67, the agent hit the 0.6M milestone. Incredibly, it was STILL surviving for exactly 401 steps, doing the Unkillable Spinning Top exploit. Its reward was `-632.9`.
* **Root Cause**: The agent willingly accepted the `-50.0` steering penalty (for holding the wheel at full lock while spinning for 400 steps). But why did PPO prefer this over crashing early (which yielded `-435.0`)? Because of `action_drive_reward = 0.5`! The agent discovered that if it held the gas pedal all the way down (`drive=1.0`) while spinning, it earned `+0.5` points per step. Over 400 steps, this yielded `+200.0` points! This massive `+200` points acted as a survival bonus in disguise, offsetting the steering penalty and making spinning mathematically superior to crashing early.
* **Fix**: Disabled `action_drive_reward` (set weight to `0.0`). The agent now receives absolutely no points for merely holding the gas pedal. It must translate that gas into physical forward progress (waypoint progress) to earn points. Without the `+200` gas pedal points, spinning in circles yields `-670`, making it mathematically worse than just swerving into the wall and crashing quickly. The only way out of this valley of death is to drive straight and earn the massive `+50` waypoint progress rewards!
* **Result**: Wiped checkpoints and restarted training.

### Issue 69: The Suicide Loophole (Stationary Penalty Over-Saturation)
* **Problem**: The agent is crashing early at ~100 steps (WPs_cum: 1, Spd: ~0.12).
* **Root Cause**: In `arcpro_env_cfg.py`, the `stationary` reward weight was set to `100.0`. Since the stationary penalty checks waypoint progress every 10 steps and returns `-5.0` if progress is low, this resulted in a massive `-500.0` penalty *every 10 steps*. Since crashing yields a flat `-500.0` terminal penalty, it is mathematically better to crash immediately (-500) than to drive slowly and survive, taking `-500` repeatedly.
* **Fix**: Reduced `stationary` penalty weight in `arcpro_env_cfg.py` from `100.0` to `10.0`. This yields a manageable `-50.0` penalty every 10 steps, punishing stagnation but preventing immediate suicide.
* **Result**: Wiped checkpoints and restarted training.

### Issue 70: Instant Gate Termination & Negative Heading Reward (Yaw De-sync)
* **Problem**: The agent was dying at exactly ~62 steps with a negative reward (`~ -35`), speed ~`0.18 m/s`, and `WPs_cum: 0`.
* **Root Cause**: In `terminations.py`, the `yaw` angle was extracted directly from the physics engine without applying the `+ pi` (180 degrees) rotation fix that was applied in `observations.py` (Issue 31). Since the root X-axis points backward, the raw `yaw` caused `head_err` to be ~180 degrees. This broke two things:
    1. The `heading_alignment_reward` (weight 10.0) in `rewards.py` used this broken `head_err`, giving the agent a massive negative reward (`-10 * speed`) for driving straight!
    2. The `white_line_contact` gate check in `terminations.py` used this broken `head_err` to determine if the agent was "aligned" with the track. Since it evaluated to False, and the forward velocity check was also missing a negation sign, the agent was flagged as "crashing backwards into a gate" the instant it reached the first gate at step ~62.
* **Fix**: Applied the same `+ pi` yaw fix and bi-directional track correction to `terminations.py`. Also negated the velocity check (`vel = -asset...`) so it correctly identifies forward movement.
* **Result**: Wiped checkpoints and restarted training.

### Issue 71: The Sparse Reward Stagnation Trap (Missing Steering Penalty)
* **Problem**: The agent was failing to learn to drive forward, choosing to either randomly park (Len: 301, Rew: -271) or randomly steer into the wall (Len: ~62, Rew: -36).
* **Root Cause**: The `action_steer_penalty` and `action_drive_reward` weights in `arcpro_env_cfg.py` were accidentally reverted to their pre-Issue-67 values (`0.0` and `0.5` respectively). With the steering penalty disabled (`0.0`), the agent received no immediate punishment for turning its wheels. It would wander into the warning track (-40.0 penalty/step) and crash, resulting in a very noisy and sparse reward signal. Without dense guidance to keep the wheels straight, the agent could not reliably find the massive `+50.0` waypoint progress reward, oscillating between braking and crashing.
* **Fix**: Restored `action_steer_penalty` to `-2.0` and `action_drive_reward` to `0.0`. This provides an immediate dense gradient telling the agent to keep the steering wheel straight, and forces it to earn points via waypoint progress rather than a free gas pedal survival bonus.
* **Result**: Wiped checkpoints and restarted training.

### Issue 72: Fear of Steering (Over-Penalized)
* **Problem**: The agent began crossing 1-3 waypoints but then crashed at 74-146 steps. The reward was massively negative (-208 to -981).
* **Root Cause**: In Issue 71, the `action_steer_penalty` was mistakenly restored to `-2.0` instead of `-0.5` (the intended value from Issue 67). A `-2.0` weight applies a massive penalty for turning the wheel, causing the agent to be terrified of steering. When it reaches a curve, it refuses to turn the wheel and crashes straight into the wall, preventing it from progressing.
* **Fix**: Reduced `action_steer_penalty` weight in `arcpro_env_cfg.py` from `-2.0` to `-0.5`. This provides a gentle reminder to keep the wheel straight on straightaways, but is small enough that the agent is willing to take the penalty to turn at a curve.
* **Result**: Wiped checkpoints and restarted training.

### Issue 73: The dt-Multiplier Illusion (All Rewards Deflated by 50x)
* **Problem**: The agent was hopelessly stuck in a local minimum, either parking (-268) or crashing early (-35). The mathematical incentives designed in the past 72 issues were completely failing to guide the agent.
* **Root Cause**: IsaacLab automatically multiplies all `RewTerm` outputs by the environment `dt` (0.02 for 50Hz). This means EVERY reward and penalty designed was 50 times weaker than intended! For example, the `termination_penalty` (designed to be a terrifying -500 points) was only yielding a pathetic -1.0 point penalty. With the crash penalty being so tiny, the agent had no fear of crashing, and parking (-1.0/step from boundary) was barely worse. The `+50` waypoint rewards were actually only `+1.0`, meaning they weren't strong enough to pull the agent out of the parking minimum.
* **Fix**: Multiplied all active reward weights in `arcpro_env_cfg.py` by 50 to counteract the `dt` multiplier (`0.02 * 50 = 1.0`).
  - `termination_penalty`: `10.0` -> `500.0` (Yields true -500 crash penalty)
  - `progress_reward`: `200.0` -> `10000.0`
  - `action_steer_penalty`: `-0.5` -> `-25.0`
  - `stationary`: `10.0` -> `500.0`
  - `heading`: `10.0` -> `500.0`
  - `smoothness`: `2.0` -> `100.0`
  - `boundary`: `0.4` -> `20.0`
* **Result**: Wiped checkpoints and restarted training. The agent will now face the true magnitude of its actions.

### Issue 74: Re-enabling the Gas Pedal Hint
* **Problem**: After multiplying all rewards by 50x in Issue 73, the agent stopped parking but was consistently crashing at ~62 steps, struggling to find the sparse waypoint rewards.
* **Root Cause**: The agent lacks a dense gradient to press the gas pedal. Because `action_drive_reward` was disabled (Issue 68) to prevent the Gas Pedal Exploit, the agent only earns points when it physically crosses waypoints. For a newborn exploring agent, hitting waypoints before hitting the wall is a sparse event, causing it to crash blindly.
* **Fix**: Re-enabled `action_drive_reward` with a weight of `25.0` (yielding `+0.5` per step, accounting for the 50x `dt` multiplier). The Gas Pedal Exploit is no longer a threat because the `stagnation` termination will kill the agent after 6 seconds if it doesn't cross waypoints, yielding a massive `-500` crash penalty that easily eclipses the `+150` points farmed from holding the gas. This safe, dense hint will coax the agent into driving forward early in training.
* **Result**: Wiped checkpoints and restarted training.

### Issue 75: Reverting the 50x Multiplier (Over-Penalization Collapse)
* **Problem**: The agent completely collapsed into a parking policy again, taking massive `-12,000` to `-15,000` penalties by inching forward at `0.02 m/s` until it died to stagnation or FOV limits.
* **Root Cause**: In Issue 73, I incorrectly assumed that all rewards needed to be multiplied by 50x to match the intended math (which ignored the `dt` multiplier). However, the previous weights (like `boundary = 0.4`) were already *empirically tuned* to work perfectly *with* the `dt` multiplier. By forcing the weights to match the flawed, dt-ignoring math, I made the environment 50x too punishing. The `boundary` penalty became an incinerating `-40.0` per step. The agent experienced Over-Penalization Collapse, curling into a ball because every direction yielded thousands of negative points.
* **Fix**: Reverted all reward weights in `arcpro_env_cfg.py` back to their pre-Issue 73 values (divided by 50). Crucially, kept the Gas Pedal Hint from Issue 74, but scaled its weight back down to `0.5`. This restores the empirically tuned balance while still providing the dense gradient to press the gas.
* **Result**: Wiped checkpoints and restarted training.

### Issue 76: Reverting the Gas Pedal Hint
* **Problem**: The agent was still trapped, crawling at `0.02 m/s` and dying at ~150 steps.
* **Root Cause**: Re-enabling `action_drive_reward` in Issue 74 was a mistake. As discovered in Issue 68, any positive drive reward risks the Gas Pedal Exploit (spinning) or confusing the agent into crawling. The agent doesn't need a drive reward because the `heading_alignment_reward` (which multiplies by speed) already provides a massive `+10.0` points per step for driving fast and straight. The agent just needs more training time to discover this massive gradient.
* **Fix**: Disabled `action_drive_reward` (set to `0.0`).
* **Result**: Wiped checkpoints and restarted training.

### Issue 77: Scaling the Heading Reward (Escaping the Crawl Trap)
* **Problem**: After Issue 76, the agent started learning to press the gas (speed increased from `0.02` to `0.06` over 0.6M steps), but the progress was excruciatingly slow.
* **Root Cause**: Because of the `dt` multiplier, `heading = 10.0` only yields `+0.2` points per step at `1.0 m/s`. The difference between crawling (`0.02 m/s`) and slightly faster crawling (`0.06 m/s`) is a minuscule `+0.008` points per step. This gradient is mathematically insufficient to rapidly pull PPO out of the local minimum against the noise of the boundary penalty (`-0.8` per step).
* **Fix**: Increased `heading_alignment_reward` weight from `10.0` to `100.0` (yielding `+2.0` points per step at `1.0 m/s`). This massive, safe gradient mathematically guarantees that driving fast into a wall (which yields a net `+96.0` score over 60 steps) is strictly better than crawling (`-116.0`). This will instantly slingshot the agent out of the crawling trap so it can reach the curves and discover the sparse waypoint rewards. The Spinning Top Exploit remains prevented because spinning averages `0.0` heading points while accumulating steering penalties.
* **Result**: Wiped checkpoints and restarted training.

### Issue 78: Reverting Heading Scale (Penalty Evasion Collapse)
* **Problem**: The agent completely flatlined, dropping its speed from `0.06 m/s` back to an absolute `0.00 m/s`, parking perfectly for 301 steps.
* **Root Cause**: In Issue 77, I increased the `heading_alignment_reward` to `100.0` to speed up learning. However, because `heading` evaluates to `speed * cos(head_err)`, making the forward reward 10x stronger also made the penalty for *imperfect* steering 10x stronger. When the randomly exploring agent wiggled the steering wheel while pressing the gas, it took massive negative penalties. To evade these massive penalties, the agent learned the ultimate Penalty Evasion strategy: hit the brakes and never move (`Spd: 0.00`), which mathematically guarantees a `0.0` heading penalty.
* **Fix**: Reverted `heading_alignment_reward` weight back to `10.0`. The slower, steady progress observed in Issue 76 (`0.02 -> 0.06 m/s`) was the correct, stable learning trajectory. RL takes millions of steps; attempting to artificially compress the learning curve by massively inflating weights only causes collapse.
* **Result**: Wiped checkpoints and restarted training.

### Issue 79: The Omnipresent Warning Track (Suicide Exploit)
* **Problem**: After Issue 78, the agent stopped parking and started crashing into the wall on purpose at exactly 60 steps (`Spd: ~0.20 m/s`, `Rew: ~ -70`).
* **Root Cause**: In Issue 33, the user explicitly set the warning track `threshold` in `mdp/rewards.py` to `0.40m`. However, because the lane is only `0.50m` wide, the dead center of the lane is `0.25m` from both the white and yellow lines. Since `0.25 < 0.40`, the agent is mathematically inside the warning track 100% of the time, no matter how perfectly it drives! This subjected the agent to an omnipresent `-0.8` per step penalty for existing, creating an inescapable "Death by a Thousand Cuts" trap. To minimize this constant pain, the agent's optimal policy was to commit suicide by steering into the wall as fast as possible to end the episode (`-48.0` boundary penalty) instead of surviving and accumulating more pain (`-240.0` boundary penalty).
* **Fix**: Reduced the `boundary_penalty` threshold in `mdp/rewards.py` from `0.40` to `0.20`. Since termination occurs at `0.15m` (when the wheels physically touch the line), this creates a precise 5cm warning track on each side of the lane, leaving a `10cm` safe zone in the dead center (`0.25 +- 0.05m`) where the penalty is strictly `0.0`. This restores the potential-based gradient and removes the incentive for suicide.
* **Result**: Wiped checkpoints and restarted training.

### Issue 80: Clamped Heading Scale (Restoring the Forward Gradient)
* **Problem**: After fixing the Suicide Exploit in Issue 79, the agent resumed driving (`Spd: 0.16 m/s`) but continued to wander sideways and crash into the boundary at ~60 steps. The `10.0` heading weight is too weak to provide a clear forward gradient, but Issue 78 proved that scaling it to `100.0` causes Penalty Evasion Collapse because the penalty for imperfect steering becomes paralyzing.
* **Root Cause**: `heading_alignment_reward` evaluates to `speed * cos(head_err)`. Because `cos(head_err)` can be negative, a high weight creates a massive penalty for any exploratory wiggling or slight backward rolling, paralyzing the agent into `Spd: 0.00`.
* **Fix**: Modified `heading_alignment_reward` in `mdp/rewards.py` to use `torch.clamp(torch.cos(head_err), min=0.0)`. This ensures the agent is massively rewarded for driving forward (`+2.0` points per step at 1 m/s), gets exactly `0.0` for driving backward or sideways, and is never actively punished by this specific term for exploratory steering. Safely scaled the weight back up to `100.0` in `arcpro_env_cfg.py` to provide the dense gradient needed to pull the agent out of the wandering phase. The Spinning Top Exploit remains prevented because spinning yields `0.0` heading points while still accumulating steering penalties.
* **Result**: Wiped checkpoints and restarted training.

### Issue 81: Bootstrapping the Gas Pedal (Overcoming the Speed Trap)
* **Problem**: In Issue 80, the agent survived slightly longer (up to 97 steps) but remained trapped at low speeds (`0.12 - 0.18 m/s`), ultimately wandering into the walls and crashing.
* **Root Cause**: The `heading_alignment_reward` is proportional to `speed`. At low speeds (`0.15 m/s`), the gradient is very weak (`+0.3` points per step). The agent is caught in a chicken-and-egg bootstrapping trap: it won't experience the massive `+2.0` reward until it randomly presses the gas hard enough to reach `1.0 m/s`, but it crashes from random steering wander before it ever reaches that speed.
* **Fix**: Re-enabled the `action_drive_reward` "Gas Pedal Hint" (weight `1.0`) in `arcpro_env_cfg.py`. This unconditionally rewards the agent for pressing the gas pedal, directly bootstrapping it to higher speeds. Once at higher speeds, the massive `heading` gradient will naturally teach it to stop wandering sideways. The user's previous fear of the "Gas Pedal Exploit" (Issue 68) is no longer a concern because `stagnation_termination` now strictly caps the episode at 300 steps, limiting the exploit to a mathematically inferior `+6.0` points compared to the `+170.0` available for actually driving the track.
* **Result**: Wiped checkpoints and restarted training.

### Issue 82: The "Death by a Thousand Cuts" Ghost (Simplifying the Reward Function)
* **Problem**: In Issue 81, we re-enabled the "Gas Pedal Hint" to bootstrap the agent. However, the agent learned to commit suicide again! `Len` dropped from 183 down to 83, and `Rew` improved from `-1831.7` to `-387.2`. The agent experienced a massive penalty for surviving that made living an absolute agony, causing it to intentionally steer into the walls to end the episode.
* **Root Cause**: The dense, overlapping penalty structure (`stationary_penalty`, `boundary_penalty`, etc.) created a toxic "Death by a Thousand Cuts" environment. The omnipresent noise from these complex "shaping" penalties masked the true forward gradient. The `boundary_penalty` warning track is mathematically problematic in narrow lanes, and the `stationary_penalty` is unnecessary now that a hard `stagnation_termination` correctly caps the episode at 300 steps.
* **Fix**: Disabled `boundary_penalty` and `stationary_penalty` by setting their weights to `0.0` in `arcpro_env_cfg.py`. The agent now only experiences positive rewards for driving forward (`heading`, `drive`, `progress`) and a sharp, unambiguous penalty (`-10.0`) only when it hits the wall and dies. This classic sparse-penalty formulation prevents the agent from learning that "existing is pain" and forces it to focus purely on driving the track.
* **Result**: Wiped checkpoints and restarted training.

### Issue 83: The Stationary Min-Max Trap Returns (Overcoming Bootstrapping Paralysis)
* **Problem**: After simplifying the reward function in Issue 82, the agent successfully survived to the maximum episode length of 301 steps! However, it achieved this by standing perfectly still (`Spd: 0.00`) and accepting a score of `-184.3`. 
* **Root Cause**: By removing `stationary_penalty`, the agent discovered the Stationary Min-Max Trap. Because random exploration inevitably leads to a crash (which yields `-10.0` termination penalty plus negative drive/steering penalties before the crash), the agent found that standing perfectly still to survive the full 300 steps yields a "reliable" bad score that is mathematically less painful than exploring. The `action_drive_reward` (weight `1.0`) was too weak to overcome the agent's fear of exploring.
* **Fix**: Restored `stationary_penalty` (weight `10.0`) to properly punish standing still. Crucially, increased the `action_drive_reward` "Gas Pedal Hint" weight from `1.0` to `10.0`. This provides a massive, unconditional `+0.2` points per step for pressing the gas, giving the agent the irresistible bootstrapping incentive it needs to accelerate. Once it accelerates to `1.0 m/s`, the clamped `heading` reward (weight `100.0`) will dominate (`+2.0` points per step) and naturally teach it to keep the wheels straight. The Gas Pedal Exploit is impossible because `stagnation_termination` caps spinning in circles to a measly `+60.0` points, far below the `+2000.0` potential of driving the track.
* **Result**: Wiped checkpoints and restarted training.

### Issue 84: The Survival Farming Trap (Crawling vs Crashing)
* **Problem**: In Issue 83, we successfully restored the bootstrapping incentive. However, the agent STILL learned to crawl (`Spd: 0.02 - 0.04 m/s`), surviving up to 511 steps and achieving POSITIVE rewards (up to `174.8`)! It refused to accelerate.
* **Root Cause**: The agent discovered the "Survival Farming Trap". Crawling at `0.03 m/s` yields a tiny positive reward per step from `waypoint_progress_reward` (`+0.26` points). Over 215 steps, this adds up to `+55.9` points. Conversely, driving fast (`2.0 m/s`) yields massive points per step (`+7.3`), but the randomly exploring agent inevitably crashes into the wall after just 10 steps, yielding only `+73.0` points before receiving the `-10.0` death penalty (net `+63.0`). Because RL optimizes expected value over the replay buffer (which is dominated by the long 215-step crawling episodes), the agent concluded that safe, slow crawling (`+147.9` total) is superior to risky, fast driving (`+63.0` total). The `stationary_penalty` (weight `10.0`) was too weak (`-0.1` points per step) to offset the `+0.26` points from crawling.
* **Fix**: Increased `stationary_penalty` weight from `10.0` to `100.0` in `arcpro_env_cfg.py`. This massively increases the penalty for crawling (`< 5.0` waypoints per 10 steps) to `-1.0` points per step! This ensures that crawling yields a net negative reward (`+0.26 - 1.0 = -0.74` per step), destroying the Survival Farming Trap. The agent is now forced to drive fast to avoid the massive stationary penalty, finally escaping the bootstrapping paralysis. Normal driving (`0.5 m/s`) crosses 17 waypoints per 10 steps and is completely immune to this penalty.
* **Result**: Wiped checkpoints and restarted training.

### Issue 85: The Crawling Death Spiral (Forcing the Gas Pedal)
* **Problem**: In Issue 84, we successfully destroyed the Survival Farming Trap by making crawling mathematically painful (`Rew` dropped to `-202.9`). The agent correctly realized that crawling is bad and began to shorten its episodes (`Len` dropped from 511 to 73-153) to escape the penalty. However, it STILL refused to press the gas pedal (`Spd: 0.06 - 0.14 m/s`), opting instead to randomly wiggle until it crashed into the wall to end the episode.
