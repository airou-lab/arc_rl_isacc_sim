...
## Solution
1. In `TrackManager`, implement a method to compute the curvature of the path a certain distance (lookahead horizon) ahead of the robot.
2. The curvature $\kappa$ can be approximated using the change in heading over distance ($d\theta / ds$) between the closest waypoint and a future waypoint (e.g., 2 meters ahead).
3. In `observations.py`, call this new `TrackManager` method and assign the calculated value to `obs[:, 10]`.
