...
## Solution
1. Modify `TrackManager.collect_raw_marker_points()` to export the extracted `raw_yellow_pts`, `raw_white_pts`, and `raw_gate_pts` as numpy arrays (`.npy`) or PyTorch tensors (`.pt`) to disk.
2. In `TrackManager.ensure_synced()`, add logic to check if these cache files exist.
3. If the cache exists, load the points directly from disk, bypassing the expensive USD scene traversal.
4. Add a `--rebuild_track_cache` CLI flag (or similar mechanism) to force a re-calculation if the USD map ever changes.
