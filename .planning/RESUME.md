...
**Last Updated:** 2026-08-13T23:15 CDT

---

## Current Status
- **Training is RUNNING in tmux (`training` session)**. We successfully restarted training with `start_tmux_training.sh`.
...
- We are currently monitoring this training phase to ensure the agent learns to drive around the entire track correctly.

## What This Session Accomplished

### 1. USD Mesh 180-Degree Flip Bug (RESOLVED)
...
### 2. Checkpoint Poisoning (RESOLVED)
- **Root Cause:** Training on the broken math had poisoned checkpoints up to `130k` with negative waypoint rewards.
- **Fix:** We purged the poisoned checkpoints and successfully resumed from a clean baseline (`agent_14450.pt`).
